...
describe("trendPdfFileName", () => {
  // PDF 現在是伺服器直接產生的檔案(見 pdf_report.py),不再是瀏覽器
  // 列印對話框那條路 —— 這裡只剩檔名要不要對,版面內容的測試搬到
  // tests/test_pdf_report.py 那邊,跟著真正產生 PDF 的程式碼一起測。

  it("期別轉成中文,帶著起訖日期", () => {
    assert.equal(trendPdfFileName("month", "2025-09-01", "2026-08-31"),
      "豬豬顧問-生產趨勢月報-2025-09-01~2026-08-31.pdf");
  });

  it("認不得的期別就用原始字串,不讓檔名整個消失", () => {
    assert.match(trendPdfFileName("fortnight", "2025-01-01", "2025-02-01"),
      /fortnight/);
  });

  it("跟 CSV 版共用同一份期別中文標籤", () => {
    const csv = trendCsvFileName("year", "2024-01-01", "2026-12-31");
    const pdf = trendPdfFileName("year", "2024-01-01", "2026-12-31");
    assert.equal(csv.replace(".csv", ""), pdf.replace(".pdf", ""));
  });
});
