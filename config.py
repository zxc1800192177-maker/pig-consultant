...
#
# 400 是使用者要求的(原本 200)。清單依事件類型分組、每組先只顯示最新
# 幾筆,筆數多了也不會把紀錄頁拉得很長。
MAX_RECENT_EVENTS = 400
MAX_RECENT_EVENT_DAYS = 14     # 補登前幾天的記錄時要看得到那幾天

# 不管事件日期多早,最近記進來的這幾筆一定要出現在「已記錄」裡。
#
# 使用者回報過「配種記錄完沒有出現在已記錄」,查了很久才發現原因是他補登
# 的日期在七天前以外 —— 清單是用**事件日期**篩的,而他補登的範圍不一定,
# 所以把天數放寬解決不了。資料其實有存進去(成功訊息會跳、母豬卡看得到),
# 但清單上看不到,從使用者的角度跟「沒記到」完全一樣。
#
# 用**記錄時間**(created_at)判斷「最近記進來的」,**全部種類合計**取這個
# 數目,而且只算這幾天內記的。
#
# **不要改回用 id。** 以前這裡寫的是「id 是遞增序號,id 最大的就是最新寫
# 進來的」,再加上「每一種類各取這麼多筆」,結果把使用者 9/4 以後記的配種
# 從清單上整個抹掉(回報:「配種紀錄只看得到 9/3,後面就沒有了」):
#   - id 只是各表各自的流水號,從 PigCHAMP 匯入的舊公豬、舊進場 id 一樣很大
#   - 五種類各取 60 筆就超過清單上限 200,補登又排最前面,正常紀錄全被擠掉
# 用這個場 9/06 的備份重現,200 格裡有 172 格是匯入的舊資料。詳見 server.py
# 的 _recent_events。
#
# 上限存在的理由仍然是匯入 —— 匯入三萬筆的當下,那三萬筆全都是「剛記進來
# 的」,沒有上限的話清單會被塞爆。
#
# 60 筆是使用者要求的。原本的 10 太少:批次生產一週一批,整批補登時
# 一次就會超過十幾筆,第十一筆之後又會從清單上消失 —— 那正是當初要修的
# 問題。60 筆仍然遠低於匯入的規模(三萬筆),擋得住匯入把清單塞爆。
RECENT_JUST_RECORDED = 60

# 匯入的檔案上限。同一個入口收兩種東西,而兩者的體積差一個量級:
#
#   PigCHAMP 匯出檔   32,814 筆 → 1.45 MB
#   完整備份(JSON)   33,024 筆 → 8.91 MB,包成請求後 10.42 MB
#
# 備份大得多是因為它是逐欄位的 JSON(每筆事件都帶著鍵名、id、excluded),
# 而且為了能用記事本讀而縮排。10 MB 的舊上限對它只剩 0.7% 餘裕 —— 下一次
# 備份就會被擋掉,而使用者只會看到一句「請求過大」,不知道是自己剛存下來
# 的備份放不回去。
#
# 32 MB 大約是這個場十年份的量。再大就不該走「整包塞進一個請求」這條路,
# 那時要改成分段上傳,而不是繼續把上限往上加 —— 免費方案只有 512MB 記憶
# 體,請求本體是一次整包讀進來的。
MAX_IMPORT_BYTES = int(os.environ.get("MAX_IMPORT_BYTES", str(32 * 1024 * 1024)))

# 趨勢報告一次最多幾期。切到週別、範圍又開很大的話,期數會線性增加 ——
# 100 期的報表在手機上已經滑不完,而且每一期都要重算一次全部指標。
# 120 期夠切三年的週報,或十年的月報,遠超過正常使用會選的範圍。
MAX_TREND_PERIODS = 120
